%% Part 3 - Task 4 (part3task3_extra.m)
...
save("./data/solSortedTask3_extra.mat",'solSorted'); % save sorted solutions

% Best solution
% Load best solution run
data = load(sprintf("./data/RunsTask3_extra/solRUN%02d.mat",solSorted(1,1)));
...
saveas(gcf,"./data/task3_extra_LM.png");
hold off;
y = reshape(solLM,[k,N]);
figure('units','normalized','outerposition',[0 0 1 1]);
...
ax.YGrid = 'on'; 
title(sprintf("LM algorithm | Dataset task 4 | k = %d",k));
saveas(gcf,sprintf("./data/task3_extra_sol.png"));
hold off;